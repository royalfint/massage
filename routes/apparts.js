...
router.get("/new", middleware.isLoggedIn, function(req, res) {
    ...
    res.render("apparts/new", {fc: req.session.fc, cats: cats, folder: middleware.folder});
});
...
router.post("/", middleware.isLoggedIn, function(req, res) {
        ...
        if(post.photos.length == 0) {
            ...
            return res.redirect("/apparts/new");
        }
        ...
        Appart.create(newProduct, function(err, newlyCreated){
            if(err){
                console.log(err);
            } else {
                req.session.fc = {photos: []};
...
router.get("/:id/edit", middleware.isLoggedIn, function(req, res){
    Appart.findById(req.params.id, function(err, foundProduct){
            if(err){
                req.flash("error", err.message);
                res.redirect("back");
            }else{
                console.log(foundProduct);
                res.render("apparts/edit", {product: foundProduct, cats: cats, folder: middleware.folder});
            }
        });
});
...
router.put("/:id", middleware.isLoggedIn, function(req, res){
        ...
        Appart.findByIdAndUpdate(post.id, newProduct, function(err, justUpdated){
            if(err){
                console.log(err);
            } else {
                console.log(justUpdated);
                return res.redirect("/myapparts");
            }
        });
});
